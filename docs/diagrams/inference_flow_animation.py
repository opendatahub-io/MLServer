...
Client → REST/gRPC → DataPlane → Registry → Model → Response
...
class InferenceRequestFlow(Scene):
    ...
    def construct(self):
        ...
        model.move_to(RIGHT * 2.8 + DOWN * 0.3)
        metrics.move_to(ORIGIN + DOWN * 1.8)

        components = VGroup(client, rest, grpc, dataplane, registry, model, metrics)
        ...
        a_dp_model = make_arrow(dataplane, model, BOX_RED)
        ...
        # ═══════════════════════════════════════════════════════════
        # STEP 5 — Forward to Model for inference
        # ═══════════════════════════════════════════════════════════
        s5 = step_label("⑤ model.predict(payload)", model.get_top() + UP * 0.4)
        self.play(FadeOut(s4), FadeIn(s5, shift=UP * 0.2))

        dot5, anim5 = flow_dot(dataplane, model)
        ...
        # ═══════════════════════════════════════════════════════════
        s6 = step_label("⑥ InferenceResponse → Client", UP * 2.8)
        self.play(FadeOut(s5), FadeIn(s6, shift=UP * 0.2))

        # Model → DataPlane
        dot6a, anim6a = flow_dot(model, dataplane, color=BOX_GREEN)
        self.play(FadeIn(dot6a), anim6a)
        self.play(FadeOut(dot6a))

        # DataPlane → REST → Client
        dot6b, anim6b = flow_dot(dataplane, rest, color=BOX_GREEN)
        self.play(FadeIn(dot6b), anim6b)
        self.play(FadeOut(dot6b))

        dot6c, anim6c = flow_dot(rest, client, color=BOX_GREEN)
        self.play(FadeIn(dot6c), anim6c)
        self.play(pulse(client, BOX_GREEN))
        self.play(FadeOut(dot6c))
        ...
        self.play(FadeOut(s6), FadeIn(result, shift=UP * 0.2))

        # Highlight full path
        self.play(
            a_client_rest.animate.set_color(BOX_GREEN),
            a_rest_dp.animate.set_color(BOX_GREEN),
            a_dp_model.animate.set_color(BOX_GREEN),
            run_time=0.8,
        )
        self.wait(1.5)
