...
# Internal test hooks for pytest-only trusted-runtimes overrides.
INTERNAL_TEST_HOOKS_ENV = "_MLSERVER_ENABLE_TEST_HOOKS"
INTERNAL_TEST_TRUSTED_RUNTIMES_ARTIFACT_ENV = "_MLSERVER_TEST_TRUSTED_RUNTIMES_PATH"
...
def is_valid_runtime_import_path(value: object) -> bool:
    return isinstance(value, str) and bool(RUNTIME_IMPORT_PATH_PATTERN.fullmatch(value))
...
def _canonicalize_runtime_import_path(import_path: str) -> str:
...
def _assert_trusted_runtime_import_path(import_path: str) -> None:
    if import_path not in _get_allowed_model_implementations():
        logger.warning(
            "Rejected untrusted model implementation '%s'.",
            import_path,
        )
        raise ValueError(
            f"Model implementation '{import_path}' is not in the "
...
def _get_import_path(klass: Type):
    ...
    return _canonicalize_runtime_import_path(import_path)


def _get_trusted_runtimes_artifact_path() -> str:
    if os.getenv(INTERNAL_TEST_HOOKS_ENV) == "1":
        return os.getenv(
            INTERNAL_TEST_TRUSTED_RUNTIMES_ARTIFACT_ENV,
...
def _load_image_baked_allowed_model_implementations(
    ...
    for runtime in runtimes:
        if (
            ...
            or not RUNTIME_IMPORT_PATH_PATTERN.fullmatch(runtime)
        ):
            raise ValueError(
...
class ModelSettings(BaseSettings):
    ...
    def validate_trusted_runtime(self) -> Self:
        ...
        self.implementation_ = _canonicalize_runtime_import_path(self.implementation_)
